...
@app.route('/regfunc', methods=['POST'])
def regfunct():
    pk1=request.get_json()
    print(pk1["pk"])
    blockchain.users.add(pk1["pk"])
    print(blockchain.users)
    return "user succesufully added",201
...
def new_transaction():
    ...
    if(not tx_data["author"] in blockchain.users):
        print("not a valid account")
        return "Invalid User",404
    #here verification  of user signature has to happen
    try:
        returned_text = subprocess.check_output("python3 -m secp256k1 checksig \
            -p {} \
            -m {} \
        -s {} ".format(pk,msg,sig), shell=True, universal_newlines=True)
    except:
        returned_text=False
        
    if(returned_text==False):
        print("signature is invalid")
        return "invalid Detials",404
    ...
    print("signature verified and transaction addded succesfully")
    return "Success", 201
...
def register_new_peers():
    ...
    print(node_address,"is added into peers")
...
def register_with_existing_node():
    ...
    if response.status_code == 200:
        global blockchain
        global peers
        print("chain is global now",peers)
        ...
        print("failed to add as peers",peers)
